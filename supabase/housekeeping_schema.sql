-- Enhanced Housekeeping Schema for Taj Hotel Standards

-- Create housekeeping_staff table FIRST (before foreign key constraint)
CREATE TABLE IF NOT EXISTS housekeeping_staff (
    ...
    shift VARCHAR(20) DEFAULT 'Day', -- Day, Night, Flexible
    status VARCHAR(20) DEFAULT 'Active', -- Active, On Leave, Inactive
    specialization VARCHAR(100), -- Regular Cleaning, Deep Cleaning, VIP Service, Maintenance
...
ADD COLUMN IF NOT EXISTS estimated_duration INTEGER DEFAULT 30, -- in minutes
...
-- Add foreign key constraint after staff table exists
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'housekeeping_assigned_staff_id_fkey'
    ) THEN
        ALTER TABLE housekeeping 
        ADD CONSTRAINT housekeeping_assigned_staff_id_fkey 
        FOREIGN KEY (assigned_staff_id) 
        REFERENCES housekeeping_staff(id) 
        ON DELETE SET NULL;
    END IF;
END $$;

-- Insert default checklist templates
INSERT INTO housekeeping_checklist_template (task_type, checklist_items) VALUES
('Regular Cleaning', '[
